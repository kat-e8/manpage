...
# Shared client for the common case: no per-call docker_host override.
# When docker_host is given, get_docker_client() builds a throwaway client
# scoped to that one call instead (ssh:// only -- see get_docker_client).
docker_client = DockerClient()


def get_docker_client(docker_host: str | None = None) -> DockerClient:
    """Return a DockerClient for this call.

    With no override, returns the shared default client (local engine / whatever
    DOCKER_HOST is set in this process's environment). With docker_host given,
    builds a new client scoped to that one call -- ssh:// targets only, so no
    TLS key material ever has to pass through a tool argument.
    """
    if not docker_host:
...
class DockerHandlers:
    ...
    async def handle_create_container(arguments: Dict[str, Any]) -> List[TextContent]:
        try:
            ...
            client = get_docker_client(arguments.get("docker_host"))
            ...
            async def pull_and_run():
                if not client.image.exists(image):
                    await asyncio.to_thread(client.image.pull, image)
    ...
    async def handle_deploy_compose(arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            docker_host = arguments.get("docker_host")
            ...
            try:
                result = await DockerHandlers._deploy_stack(compose_path, project_name, debug_info, docker_host)
    ...
    @staticmethod
    async def _deploy_stack(compose_path: str, project_name: str, debug_info: List[str], docker_host: str | None = None) -> str:
        compose = DockerComposeExecutor(compose_path, project_name, docker_host)
    ...
    async def handle_get_logs(arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            debug_info.append(f"Fetching logs for container '{
                              container_name}'")
            logs = await asyncio.to_thread(client.container.logs, container_name, tail=100)
    ...
    async def handle_list_containers(arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            client = get_docker_client((arguments or {}).get("docker_host"))
